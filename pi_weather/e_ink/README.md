...
2. **System build dependencies**, needed before `pip install -r
   requirements-ink.txt` will succeed — `RPi.GPIO`, `spidev`, and `lgpio`
   ...
   (`cannot find -llgpio`).
3. **`lgpio` is required, not optional**, despite not being in the original
